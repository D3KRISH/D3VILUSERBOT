...
from . import *
from mafiabot.sql import pmpermit_sql as pm_sql
...
MAFIA_ZERO = "Go get some sleep retard. \n\n**Blocked !!**"
MAFIA_FIRST = (
    "**🔥 𝔇3𝔳𝔦𝔩𝔅𝔬𝔱 Prîvã†é Sêçürïty Prø†öçõl 🔥**\n\nThis is to inform you that "
    "{} is currently unavailable.\nThis is an automated message.\n\n"
    "{}\n\n**Please Choose Why You Are Here!!**".format(mafia_mention, CSTM_PMP)
)

@bot.on(mafia_cmd(pattern="block$"))
...
if PM_ON_OFF != "DISABLE":
    ...
    @bot.on(mafia_cmd(pattern="(a|approve|allow)$"))
    ...
    @bot.on(mafia_cmd(pattern="(da|disapprove|disallow)$"))
    ...
    @bot.on(mafia_cmd(pattern="listapproved$"))
    async def approve_p_m(event):
        if event.fwd_from:
            return
    ...
    async def do_pm_permit_action(chat_ids, event):
        ...
        if PM_WARNS[chat_ids] == Config.MAX_SPAM:
            r = await event.reply(MAFIA_ZERO)
            await asyncio.sleep(3)
