...
@bot.on(mafia_cmd(pattern="autoname"))
async def _(event):
    if event.fwd_from:
        return
...
@bot.on(mafia_cmd(pattern="autobio"))  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
...
@bot.on(mafia_cmd(pattern="reserved", outgoing=True))
