...
@bot.on(mafia_cmd(pattern="rename (.*)", outgoing=True))
@bot.on(sudo_cmd(pattern="rename (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    mafia = await eor(event, 
    ...
    if event.reply_to_msg_id:
        ...
        if os.path.exists(downloaded_file_name):
            await mafia.edit("Downloaded to `{}` in {} seconds.".format(downloaded_file_name, ms)
            )
        else:
            await eod(mafia, "Error Occurred\n {}".format(input_str))
    else:
        await eod(mafia, f"Syntax `{hl}rename file.name` as reply to a Telegram media")


@bot.on(mafia_cmd(pattern="rnupload (.*)", outgoing=True))
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        if os.path.exists(downloaded_file_name):
            ...
            await mafia.edit("Downloaded in {} seconds. Uploaded in {} seconds.".format(
                    ms_one, ms_two
                )
            )
        else:
        ...
        await mafia.edit("Syntax // `{}rnupload file.name` as reply to a Telegram media".format(hl))


@bot.on(mafia_cmd(pattern="rnsupload (.*)", outgoing=True))
        ...
        if os.path.exists(downloaded_file_name):
            ...
            try:
                await bot.send_file(
                    ...
                    downloaded_file_name,
                    thumb=thumb,
                    caption="reuploaded by [HellBot](https://t.me/mafiabot_official_chat)",
                    force_document=False,
                    allow_cache=False,
                    reply_to=event.message.id,
                    attributes=[
            ...
            await eod(mafia, "File Not Found {}".format(input_str))
    else:
        await mafia.edit(
