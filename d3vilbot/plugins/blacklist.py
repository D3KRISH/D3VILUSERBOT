...
from mafiabot.sql import blacklist_sql as sq
from . import *
...
@bot.on(mafia_cmd(pattern="addblacklist ((.|\n)*)"))
...
@bot.on(mafia_cmd(pattern="rmblacklist ((.|\n)*)"))
...
@bot.on(mafia_cmd(pattern="listblacklist$"))
...
CmdHelp("blacklist").add_command(
  "addblacklist", "<word>/<words>", "The given word or words will be added to blacklist in that specific chat if any user sends then the message gets deleted.\n\nNote :- If you are adding more than one word at time via this, then remember that new word must be given in a new line that is not [hi mafiao]. It must be as [hi \n mafiao]"
).add_command(
