...
from mafiabot.helpers import runner
from . import *
...
@bot.on(mafia_cmd(outgoing=True, pattern=r"update(| now)$"))
...
async def upstream(event):
    ...
    if conf == "" and not force_update:
        ...
        await event.delete()
        return await event.respond(f"🌚 Do `{hl}update build` to update your ** ∂3vιℓвσт** !!")

    if force_update:
        await event.edit(
            "`Force-Updating  ∂3vιℓвσт. Please wait...`"
        )
    if conf == "now":
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        await event.edit(f"**Your  ∂3vιℓвσт Is UpToDate**\n\n**Version :**  __{mafia_ver}__\n**Oɯɳҽɾ :**  {mafia_mention}")
    else:
        await event.edit("**Please set up**  `HEROKU_API_KEY`  **from heroku to update!**")
    return


@bot.on(mafia_cmd(outgoing=True, pattern=r"update build$"))
...
async def upstream(event):
    ...
    ups_rem = repo.remote("upstream")
    ups_rem.fetch(ac_br)
    await event.edit(f"**∂3vιℓвσт Docker Build In Progress... Type** `{hl}ping`  **after 5 mins to check if Bot is working!**")
