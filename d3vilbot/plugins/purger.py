...
@bot.on(mafia_cmd(pattern=r"purge", outgoing=True))
...
@bot.on(mafia_cmd(pattern=r"purgeme", outgoing=True))
...
@bot.on(mafia_cmd(pattern=r"sd", outgoing=True))
...
CmdHelp("purger").add_command(
  ...
  "sd", "<time> <text>", "Sends a self destruct text. Fill time in secs", "sd 10 mafiao"
).add_command(
